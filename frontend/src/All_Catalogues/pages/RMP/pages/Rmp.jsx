...
const EmptyState = ({ query }) => (
  <div className="empty-state">
    ...
    <p className="empty-title">No matching Item found</p>
    {query && (
...
const TableRow = ({ item, query, index }) => (
  <tr className="table-row" style={{ animationDelay: `${index * 25}ms` }}>
    ...
    <td className="td-filter">{item.itemDescription || "—"}</td>
...
const FilterCard = ({ item, query, index }) => (
  <div className="filter-card" style={{ animationDelay: `${index * 40}ms` }}>
    ...
    <div className="card-title">{item.itemDescription || "—"}</div>

    <div className="card-desc">{highlight(item.application || "—", query)}</div>

    <div className="card-footer">
      <span className="footer-label">Type</span>
      <span className="footer-value">{item.filterType || "—"}</span>
      <span className="footer-label">Brand</span>
      <span className="footer-value">{item.brand || "—"}</span>
    </div>
  </div>
);
    ...
    <div className="bosch-container">
      ...
      <div className="top-bar">
        <div className="title-group">
          ...
          <div>
            <h2 className="page-title">RMP Bearings</h2>
            <p className="page-subtitle">
              {hasData
                ? `${finalData.length} Bearing${finalData.length !== 1 ? "s" : ""} available`
