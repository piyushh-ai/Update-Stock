// BoschStarter.jsx — Enhanced with Detail Modal
...
const EmptyState = ({ query }) => (
  <div className="empty-state">
    ...
    <p className="empty-title">No matching starters found</p>
    {query && (
...
const DetailModal = ({ item, isOpen, onClose, loading }) => {
  ...
  // Section groupings for detail modal
  const coreInfo = [
  ...
  const technicalComp = [
    { label: "CES", value: data.CES || "—", badge: true },
    { label: "DEF", value: data.DEF || "—", badge: true },
    { label: "EGT", value: data.EGT || "—", badge: true },
    { label: "ORC", value: data.ORC || "—", badge: true },
    { label: "Stator Frame", value: data.StatorFrame || "—", badge: true },
    { label: "Armature", value: data.armature || "—", badge: true },
  ];

  const bearingElec = [
    { label: "Bearing CES", value: data.bearingCES || "—", badge: true },
    { label: "Bearing DEF", value: data.bearingDEF || "—", badge: true },
    { label: "Brush Holder", value: data.brushHolder || "—", badge: true },
    { label: "Solenoid Switch", value: data.solenoidSwitch || "—", badge: true },
  ];
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header-desktop">
          <div className="modal-icon">
            <svg viewBox="0 0 64 64" fill="none">
              <circle cx="32" cy="32" r="30" fill="var(--color-accent-subtle)" opacity="0.5" />
              <rect x="16" y="20" width="32" height="24" rx="4" fill="var(--color-accent)" opacity="0.15" />
              <rect x="16" y="20" width="32" height="24" rx="4" stroke="var(--color-accent)" strokeWidth="1.5" />
              <circle cx="45" cy="32" r="2.5" fill="var(--color-accent)" />
              <path d="M22 28h12M22 32h10M22 36h8" stroke="var(--color-accent)" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </div>
        ...
        <div className="modal-sections">
          ...
          <div className="modal-sections-desktop">
            ...
            <DetailSection title="Technical Components" items={technicalComp} />
...
const BoschStarter = () => {
  ...
  /* Derived states */
  const finalData = data?.starterData || [];
    ...
    <div className="bosch-container">
      ...
      <div className="top-bar">
        <div className="title-group">
          ...
          <div>
            <h2 className="page-title">Bosch Starters</h2>
            <p className="page-subtitle">
              {hasData
                ? `${finalData.length} starter${finalData.length !== 1 ? "s" : ""} available`
                : "Bosch automotive starters"}
            </p>
          </div>
        </div>
